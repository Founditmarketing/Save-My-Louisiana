...
export const Section: React.FC<Props> = ({ data, perspective, index, isActive }) => {
  ...
  return (
    ...
    >
       ...
       <div className={`absolute left-6 md:left-1/2 top-40 transform -translate-x-1/2 w-4 h-4 rounded-full border-2 transition-colors duration-500 z-10 hidden md:block ${
           isActive ? 'border-brand-red scale-125 bg-white' : 'border-gray-300 bg-white'
       }`}></div>
        ...
        <div className={`flex flex-col lg:flex-row gap-12 lg:gap-24 items-center ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
          ...
          <div className="w-full lg:w-1/2">
             <div className={`relative group transition-all duration-1000 delay-200 ease-out ${hasEntered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
                ...
                <div className={`absolute top-4 left-4 right-4 bottom-4 border-2 rounded-[2.5rem] z-0 transition-transform duration-700 ${
                    isActive ? 'scale-105' : 'scale-100'
                } border-brand-blue/20`} />
                
                {/* Main Image Container */}
                <div className="relative overflow-hidden rounded-[2.5rem] shadow-2xl aspect-[4/3] bg-gray-100">
                    <img 
                      src={data.imageSrc} 
                      alt={data.imageAlt}
