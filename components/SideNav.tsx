...
export const SideNav: React.FC<Props> = ({ sections, activeId }) => {
  ...
  return (
    ...
    <div className="fixed right-2 md:right-6 top-1/2 transform -translate-y-1/2 z-50 flex flex-col gap-4 md:gap-6 items-end pointer-events-none">
      ...
      {sections.map((section) => {
        ...
        return (
          ...
          >
            ...
            <span className={`hidden sm:block absolute right-12 text-[10px] font-bold uppercase tracking-widest bg-brand-dark/40 backdrop-blur-md text-white px-3 py-1.5 rounded shadow-lg transition-all duration-500 whitespace-nowrap border border-white/10 ${
              isTimerVisible
            ...
            <div className="relative flex items-center justify-center w-8 h-8 md:w-10 md:h-10">
                ...
                {/* Button Itself */}
                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center shadow-md transition-all duration-300 border-2 ${
                  isActive 
