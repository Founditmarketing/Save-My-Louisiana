                ...
                <div className="grid lg:grid-cols-2 gap-16 items-start mb-16">
                    ...
                    <div className="space-y-8">
                        <div className="rounded-2xl overflow-hidden shadow-xl">
                            <img
                                src="/donate-community.png"
                                alt="Louisiana community members standing together at a town hall meeting"
                                className="w-full h-auto object-cover"
                            />
                        </div>

                        <div className="bg-brand-dark text-white p-8 rounded-2xl">
                            <h3 className="text-xl font-heading font-bold mb-3">Every Dollar Counts</h3>
                            <p className="text-gray-300 font-serif leading-relaxed">
                                100% of donations go directly to legal defense, expert witnesses, and community advocacy. Together, we can protect Louisiana's land, water, and future generations.
                            </p>
                        </div>
                    </div>
                </div>

                {/* Dynamic GoFundMe Block — managed from Sanity CMS */}
                {settings.gofundmeEnabled && settings.gofundmeUrl && (
                    <div className="max-w-2xl mx-auto mb-12">
                        <div className="bg-gradient-to-br from-green-600 to-green-700 text-white rounded-2xl p-8 text-center shadow-xl">
                            ...
                            <h3 className="text-2xl font-heading font-bold mb-2">
                                {settings.gofundmeCampaignTitle || 'Support Our Campaign'}
                            </h3>
                            {settings.gofundmeDescription && (
                                <p className="text-green-100 font-serif mb-6 leading-relaxed">
                                    {settings.gofundmeDescription}
                                </p>
                            )}
                            {settings.gofundmeGoalAmount && (
                                <p className="text-sm text-green-200 mb-4 font-bold uppercase tracking-widest">
                                    Goal: ${settings.gofundmeGoalAmount.toLocaleString()}
                                </p>
                            )}
                            <a
                                ...
                                className="inline-flex items-center gap-2 bg-white text-green-700 hover:bg-green-50 px-8 py-3 rounded-full font-bold uppercase tracking-widest text-sm transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                            >
                                Donate on GoFundMe <ExternalLink size={16} />
                            </a>
                        </div>
                    </div>
                )}

                {/* Donation Form — Seamlessly embedded as part of the page */}
                <div className="max-w-2xl mx-auto">
                    <h2 className="text-3xl font-heading font-bold text-gray-900 text-center mb-2">Make Your Donation</h2>
                    <p className="text-gray-500 text-center mb-8 font-serif">Secure payment processed by Anedot.</p>
                    <iframe
